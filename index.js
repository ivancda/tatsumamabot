...
client.on("messageCreate", async (message) => {
  ...
  if (cmd === "!coloca") {
    ...
    try {
      ...
      if (isNaN(time) || time < 0)
        return message.reply("⚠️ Envie um número válido de segundos!");
  ...
  if (cmd === "!toca") {
    ...
    if (finalQuery.includes("youtube.com") && finalQuery.includes("&")) {
      finalQuery = finalQuery.split("&")[0]; // remove parâmetros como &list=
    }
    ...
    if (/spotify\.com\/.*\/track/.test(finalQuery)) {
      try {
        ...
        if (!youtubeLink)
          return message.reply("❌ Não encontrei a música no YouTube.");
